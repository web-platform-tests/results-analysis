 ...
 * repository (results-analysis-cache).
 *
 * The results-analysis-cache repository stores results from WPT runs as individual
...
// Verifies that every run in |alignedRuns| is present in |repo|, then loads
// each run's results onto its 'tree' field. |alignedRuns| is a map of date to
// list of runs, as returned by lib.runs.fetchAlignedRunsFromServer.
async function loadRunTrees(repo, alignedRuns) {
  ...
  let before = Date.now();
  const localRunIds = await getLocalRunIds(repo);
  let after = Date.now();
  ...
  for (const runs of alignedRuns.values()) {
    for (const run of runs) {
      ...
      run.tree = await getGitTree(repo, run);
    }
  }
  after = Date.now();
