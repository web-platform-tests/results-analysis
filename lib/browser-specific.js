...
function scoreSubtests(browserSubtests) {
  ...
  while (browserSubtests.some(subtests => subtests.hasCurrent())) {
    const [name] = findSmallestNameAndIndex(browserSubtests);
    const onSameSubtest = browserSubtests.filter(s => s.hasCurrent() &&
                                                 s.value().name == name);

    if (name === prevName) {
      // There actually (rarely) exist distinct subtests with the same name in
      // the data, usually because of unprintable characters. This can influence
      // the result as we may mismatch results (i.e. if some browser has results
      // for one duplicate-named subtest but not another).
      //
      // We avoid scoring another subtest result as otherwise we end up with the
      // denominator being greater than the number of unique subtests, which is
      // a very surprising outcome. Thus, in this branch, do nothing.
    } else if (onSameSubtest.length < browserSubtests.length) {
      // At this point at least one browser is missing a test that at least
      // one other browser has. This could be a browser-specific failure, if
      // exactly N-1 browsers have a passing state for that test (as we are
      // treating missing as a failure state).
      denominator += 1;
      if (onSameSubtest.length == browserSubtests.length - 1 &&
          onSameSubtest.every(s => TEST_PASS_STATUSES.includes(
              s.value().status))) {
        for (let i = 0; i < browserSubtests.length; i++) {
          if (!browserSubtests[i].hasCurrent() ||
              browserSubtests[i].value().name != name) {
            counts[i] += 1;
            break;
          }
        }
