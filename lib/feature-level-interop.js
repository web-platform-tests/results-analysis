...
// Counts the products of |expectedBrowsers| that pass any part of one test,
// from the |scores| that test produced. A product with no result for the test
// has not passed it.
function passingProductCount(scores, expectedBrowsers) {
  let passing = 0;
  for (const product of expectedBrowsers) {
    if (scores.has(product) && scores.get(product) > 0) {
      passing += 1;
    }
  }
  return passing;
...
function scoreFeature(runs, expectedBrowsers, tests) {
  ...
  for (const test of tests) {
    ...
    // A test exactly one product passes is scope only that product has
    // implemented, and counting it lowers every other product as the feature
    // grows, which reads as a regression none of them caused. A test no product
    // passes is kept: that is a feature nobody implements yet.
    if (passingProductCount(scores, expectedBrowsers) === 1) {
      continue;
    }
    testCount += 1;
